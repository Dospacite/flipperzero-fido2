...
const uint8_t* u2f_get_device_key(const U2fData* U2F) {
    furi_assert(U2F);
    return U2F->device_key;
}

uint32_t u2f_get_counter(const U2fData* U2F) {
    furi_assert(U2F);
    return U2F->counter;
}

uint32_t u2f_advance_counter(U2fData* U2F) {
    furi_assert(U2F);
    U2F->counter++;
    if(!u2f_data_cnt_write(U2F->counter)) {
        FURI_LOG_E(TAG, "Counter write failed");
    }
    if(U2F->callback != NULL) U2F->callback(U2fNotifyAuthSuccess, U2F->context);
    return U2F->counter;
