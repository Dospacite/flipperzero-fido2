...
#define U2F_HID_CBOR   (U2F_HID_TYPE_INIT | 0x10) // Send CTAP2 CBOR message
...
static bool u2f_hid_parse_request(U2fHid* u2f_hid) {
    ...
    if(u2f_hid->packet.cmd == U2F_HID_PING) { // PING - echo request back
        ...
        if((u2f_hid->lock == true) && (u2f_hid->packet.cid != u2f_hid->lock_cid)) return false;
        bool is_assertion = u2f_hid->packet.len > 0 && u2f_hid->packet.payload[0] == 0x02;
        uint32_t counter = u2f_get_counter(u2f_hid->u2f_instance) + (is_assertion ? 1 : 0);
        uint16_t response_len =
            ctap2_handle(u2f_hid->ctap2, u2f_hid->packet.payload, u2f_hid->packet.len, counter);
        if(response_len == 0) return false;
        if(is_assertion && u2f_hid->packet.payload[0] == 0x00) {
            u2f_advance_counter(u2f_hid->u2f_instance);
        }
        u2f_hid->packet.len = response_len;
        u2f_hid_send_response(u2f_hid);

    } else if(u2f_hid->packet.cmd == U2F_HID_CANCEL) {
        /* Requests are processed synchronously, so there is no pending operation to cancel. */
        ...
        u2f_hid->packet.payload[16] = 1 | 4; // Capabilities: wink + CBOR + MSG
        u2f_hid_send_response(u2f_hid);
...
U2fHid* u2f_hid_start(U2fData* u2f_inst) {
    ...
    if(!u2f_hid) return NULL;
    memset(u2f_hid, 0, sizeof(*u2f_hid));
